...
public class Board : IBoard
{
    ...
    public bool Contains(string name)
        => this.cardsByName.ContainsKey(name);

    public int Count()
        => this.cardsByName.Values.Count;

    public void Play(string attackerCardName, string attackedCardName)
    {
        if (!this.cardsByName.ContainsKey(attackerCardName) || !this.cardsByName.ContainsKey(attackedCardName) || this.cardsByName[attackerCardName].Level != this.cardsByName[attackedCardName].Level)
        {
            throw new ArgumentException();
        }
    ...
    {
        var cardsForRemove = this.cardsByName.Values.Where(c=>c.Health <= 0).ToList();
